...
# Thin compatibility shim for flax.optim-style optimizer API using optax
...
# Register as a JAX pytree so flax_utils.replicate / jax.pmap can handle it.
# Dynamic leaves: (params, opt_state); static aux: the optax transform _tx.
...
def init_model(rng, config):
  ...
  # In newer Flax, model.init returns a plain dict; pop returns just the value
  initial_params = variables.pop('params')
  init_model_state = variables  # remaining keys (e.g. batch_stats) are the model state
